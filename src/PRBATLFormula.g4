...
state_formula : '(' state_formula ')' | 'T' | proposition | 'not' state_formula | state_formula 'or' state_formula |
                '<<' agents '^' bounds '>>' ('prob' comp_op real_number)? path_formula;

proposition : NAME;

agents : '{' (POSITIVE_NUMBER (',' POSITIVE_NUMBER)*)? '}';
